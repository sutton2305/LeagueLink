...
const Dashboard: React.FC<DashboardProps> = ({ league, currentUser, onRolloverLeague, onUpdateLeague, onDeleteLeague }) => {
  const [activeTab, setActiveTab] = useState<Tab>('Home');
  
  const [players, setPlayers] = useState(() => db.getPlayers(league.id));
  const [teams, setTeams] = useState(() => db.getTeams(league.id));
  const [matches, setMatches] = useState(() => db.getMatches(league.id));
  const [scheduledMatches, setScheduledMatches] = useState(() => db.getScheduledMatches(league.id));
  const [chatMessages, setChatMessages] = useState(() => db.getChatMessages(league.id));
  const [brackets, setBrackets] = useState(() => db.getBrackets(league.id));
  
  useEffect(() => { db.savePlayers(league.id, players); }, [league.id, players]);
  useEffect(() => { db.saveTeams(league.id, teams); }, [league.id, teams]);
  useEffect(() => { db.saveMatches(league.id, matches); }, [league.id, matches]);
  useEffect(() => { db.saveScheduledMatches(league.id, scheduledMatches); }, [league.id, scheduledMatches]);
  useEffect(() => { db.saveChatMessages(league.id, chatMessages); }, [league.id, chatMessages]);
  useEffect(() => { db.saveBrackets(league.id, brackets); }, [league.id, brackets]);
  ...
  useEffect(() => {
    const checkStorage = () => {
        setIsMatchInProgress(!!db.getLiveMatchState(league.id));
    };

    checkStorage();

    const handleStorageChange = (event: StorageEvent) => {
        if (event.key === `live_match_state_${league.id}`) {
            checkStorage();
        }
    };
  ...
  const tabs: { name: Tab, icon: React.FC<{className?: string}> }[] = [
    ...
    { name: 'Player Stats', icon: UsersIcon },
